...
def compute(patch):
    ...
    diff_diagonal += np.sum(np.abs(patch[1:, :-1, :] - patch[:-1, 1:, :]))
    result = diff_horizontal + diff_vertical + diff_diagonal
    return result.sum()
...
def bit_patch(img, img_height, bit_mode, patch_size, patch_mode):
    ...
    resize_op = transforms.Resize((img_height, img_height))
    if min_len < patch_size:
        combined_image = resize_op(combined_image)

    num_patch = (img_height // patch_size) ** 2
    patch_list = []
    random_crop = transforms.RandomCrop(patch_size)
    for _ in range(num_patch):
        patch_list.append(random_crop(combined_image))
